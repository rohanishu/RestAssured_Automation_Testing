...
public class UserTestsUsingProp {
	...
	{
		Response res=UserEndPoints2.readUser(this.user.getUsername());
		res.then().log().all();
		Assert.assertEquals(res.getStatusCode(),200);
	}
	...
	public void testupdateUser()
	{
		...
		Response res=UserEndPoints2.updateUser(this.user.getUsername(), user);
		
		Assert.assertEquals(res.getStatusCode(), 200);
		
		//checking the updated username by passing the details getUser
		Response resafterUpdate=UserEndPoints2.readUser(this.user.getUsername());
		res.then().log().all();
